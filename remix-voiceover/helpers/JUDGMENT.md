...
- Keep residual overlays inside every `plan_structure` limit in `scripts/rv/audio-policy.json`. Give every overlay a distinct `event_reason` and structured `event_citation`: `{source: analysis, ref: /step_candidates/0}` or `{source: promotion_manifest, path: prior/promotion_manifest.json, ref: /rows/0}`. A prior promotion citation may point to a passing or failing row, but its source and analysis hashes must match the current run. The cited object must carry event time, and the overlay must stay inside its owning baseline and configured evidence neighborhood.
...
- Use `scripts/rv/audio-policy.json` as the runtime authority for the house audio rails it defines. Its commentary target, mic-over-bed gap, sustained-masking minimum, and true-peak ceiling are workflow calibrations, not universal standards; see `references/AUDIO-ENGINEERING.md`.
...
- Treat `clean_mic_gain_ceiling_db.default` as an evidence-and-audition trigger, not a physical damage threshold. Above it, cite the affected regime's `clean_gain_headroom`; the legal ceiling is that regime's measured maximum minus the configured safety margin, subject to true peak and peak-control checks.
...
- Enable declared mic-only peak control when large clean regime lifts over expressive speech create isolated yell or emphasis peaks while ordinary speech BODY is already aligned. Use `render.peak_control` with `mechanism: alimiter` and a ceiling inside validated bounds; do not use it to raise loudness, flatten sustained delivery, control the bed, or limit the mix. Verification must satisfy every `peak_control_accounting` rail in `scripts/rv/audio-policy.json` so control remains sparse and does not reshape sustained speech.
...
- Use `rails.rails_adjustment` only inside the bounds in `scripts/rv/audio-policy.json`; cite exact `analysis.json` fields.
