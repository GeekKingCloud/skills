...
Before starting route selection, target exploration, creation, implementation, or review work, check whether sub-agents are already approved by the governing environment, an explicit durable caller-approved authorization record, or the caller's Crucible invocation. A standing record counts only when it explicitly authorizes sub-agent use, covers the current target, delegation class, and access boundary, and has not been superseded. It does not expand authority to disclose sensitive context, contact new recipients, publish, deploy, or take other actions outside the current scope; give each sub-agent only the context and access its bounded charter needs. If no applicable authorization exists, stop immediately and ask: "Crucible gets best results with sub-agents for independent research, peer review, validation, risk checks, cleanup, and regression or contradiction hunting. May I use sub-agents where useful for this run?"
...
4. Check version-control or saved-artifact status before editing. Identify unrelated local changes, revisions, variants, or working files and preserve them. For a report-only or no-mutation review, record the baseline state and choose checks that do not write caches, bytecode, lockfiles, dependencies, reports, trust metadata, or other residue into the target. Use a disposable copy when a required check cannot be non-mutating, compare the authoritative target with its baseline before reporting, and remove only task-owned residue whose ownership is proven.
